...
SELECT
	...
	AR.BKR_BOOKING,
	MAX(CAST(AR.CREATED_DATE as DATETIME))	AS [BOOK_DAY_BAG],
...
SELECT 
	...
	CAST(BS.[CREATED_DATE] as DATE)							AS [BOOKING_DATE],
	BS.[CREATED_DATE]										AS [BOOKING_DATETIME],
	BS.[DEPARTURE_AIRPORT],																/** check- to drop **/
	BS.[ARRIVAL_AIRPORT],																/** check- to drop **/
	APA.[COUNTRY_CODE]										AS [COUNTRY_CODE_ARR],
	APD.[COUNTRY_CODE]										AS [COUNTRY_CODE_DEP] ,
	CASE	WHEN (BS.[DEPARTURE_AIRPORT] = 'AMS'	OR BS.[ARRIVAL_AIRPORT] = 'AMS')	THEN 1 --'AMS'
			WHEN (BS.[DEPARTURE_AIRPORT] = 'EIN'	OR BS.[ARRIVAL_AIRPORT] = 'EIN')	THEN 2 --'EIN'
			WHEN (BS.[DEPARTURE_AIRPORT] = 'RTM'	OR BS.[ARRIVAL_AIRPORT] = 'RTM')	THEN 3 --'RTM'
			WHEN (BS.[DEPARTURE_AIRPORT] = 'GRQ'	OR BS.[ARRIVAL_AIRPORT] = 'GRQ')	THEN 4 --'GRQ'
			ELSE 'NONE' 
			END AS [HUB_NL],
	MAX(CAST(PS.[DEPARTURE_DATETIME_PLS] AS DATETIME))		AS [DEPARTURE_DATETIME],														/** check- to drop **/
	MAX(CAST(PS.[DEPARTURE_DATETIME_PLS] AS DATE))			AS [DEPARTURE_DATE],			/** check- to drop **/
	CONVERT(varchar, PS.[DEPARTURE_DATETIME_PLS], 108)		AS [DEPARTURE_TIME],			/** check- to drop **/
	...
	BS.[TOTAL_FLIGHT_DURATION_IN_MIN]						AS [LENGTH_OF_STAY_IS_FLIGHTTIME],
	AR.[SOURCE],											
	AR.[BOOK_DAY_BAG]										AS [BOOKING_DATE_BAG],
	AR.[WEIGHT_OF_ITEMS],
	AR.[ANCILLARY_REVENUE],
	BO.[BKR_BOOKING],											/** check- to drop **/
	CASE	WHEN BS.[FARECLASSOFSERVICE] = 'W'		THEN 1 --GROUP
			ELSE 0 --NOGROUP
			END AS [GROUPBOOKING],
...
LEFT JOIN [DWH_COM].[VW_DIM_AIRPORT]						AS APA	ON BS.[ARRIVAL_AIRPORT] = APA.[IATA_CODE]					
																	AND APA.[BL_CRUD] != 'D' 
																	AND APA.[BL_ISCURR] = 1
LEFT JOIN [DWH_COM].[VW_DIM_AIRPORT]						AS APD	ON BS.[DEPARTURE_AIRPORT] = APD.[IATA_CODE]					
																	AND APD.[BL_CRUD] != 'D' 
																	AND APD.[BL_ISCURR] = 1
...
WHERE	1=1
		...
		AND BS.[FIRMNESS] = 'E'
		AND (BS.[ARRIVAL_AIRPORT] IN ('AMS', 'EIN', 'RTM', 'GRQ') OR BS.[DEPARTURE_AIRPORT] IN ('AMS', 'EIN', 'RTM', 'GRQ'))
...
GROUP BY 
	...
	BS.[TOTAL_FLIGHT_DURATION_IN_MIN],
...
SELECT 
	...
	[ARRIVAL_AIRPORT],
	[HUB_NL],
	CASE	WHEN [DIRECTION] = 'O' THEN  [COUNTRY_CODE_ARR] 
			ELSE [COUNTRY_CODE_DEP] 
			END AS [COUNTRY_CODE],
	CASE	WHEN [DIRECTION] = 'O' THEN 0
			ELSE 1
			END AS [DIRECTION],
	...
	CASE    WHEN [DEPARTURE_TIME]  BETWEEN '05:00:00' AND '08:00:00' THEN 1 --'Morning Early'
			WHEN [DEPARTURE_TIME]  BETWEEN '08:00:00' AND '10:00:00' THEN 2 --'Morning'
			WHEN [DEPARTURE_TIME]  BETWEEN '10:00:00' AND '12:00:00' THEN 3 --'Morning Late'
			WHEN [DEPARTURE_TIME]  BETWEEN '12:00:00' AND '14:00:00' THEN 4 --'Afternoon Early'
			WHEN [DEPARTURE_TIME]  BETWEEN '14:00:00' AND '16:00:00' THEN 5 --'Afternoon'
			WHEN [DEPARTURE_TIME]  BETWEEN '16:00:00' AND '17:00:00' THEN 6 --'Afternoon Late'
			WHEN [DEPARTURE_TIME]  BETWEEN '17:00:00' AND '19:00:00' THEN 7 --'Evening'
			WHEN [DEPARTURE_TIME]  BETWEEN '19:00:00' AND '21:00:00' THEN 8 --'Evening Late'
			ELSE 9 --'Night' 
			END AS [TIMESLOT_OF_DEPARTURE],
	[DEPARTURE_DATE],
	[BOOKING_DATE],
	DATEPART(weekday, [BOOKING_DATE])								AS [WEEKDAY_OF_BOOKING],
	DATEDIFF(minute, [BOOKING_DATETIME], [DEPARTURE_DATETIME])		AS [BOOKING_DBD],
    [BOOKING_DATE_BAG],
	DATEPART(weekday, [BOOKING_DATE_BAG])							AS [WEEKDAY_OF_BOOKING_BAG],
	DATEDIFF(minute, [BOOKING_DATE_BAG], [DEPARTURE_DATETIME])		AS [BOOKING_BAG_DBD],
	CASE	WHEN [LENGTH_OF_STAY_INCL_FLIGHTTIME] > 432000			THEN [LENGTH_OF_STAY_IS_FLIGHTTIME] --Set boundary for return flight as one booking on 300 days (GoLive dates)
			ELSE [LENGTH_OF_STAY_INCL_FLIGHTTIME]
			END AS [LENGTH_OF_STAY],
	CASE	WHEN [DIRECTION] = 'I'									THEN [SEASONALITY_INBOUND]
			ELSE [SEASONALITY_OUTBOUND]
			END AS [SEASONALITY],
	CASE WHEN [SOURCE] = 'BF' THEN 1
	        WHEN [SOURCE] = 'PFS' THEN 2
	        ELSE 0
	        END AS [SOURCE],
	[NBR_OF_PAX_IN_PNR],
...
WHERE 1=1
AND ((DATEDIFF(minute, [BOOKING_DATE_BAG], [DEPARTURE_DATETIME]) >= 0) OR (DATEDIFF(minute, [BOOKING_DATE_BAG], [DEPARTURE_DATETIME]) IS NULL))
AND DATEDIFF(minute, [BOOKING_DATETIME], [DEPARTURE_DATETIME]) >= 0
...
def dummy_columns(data, columns):
    """
    Dummy code columns that have more than two categories of missing values which form the category None

    :param pandas.DataFrame: Dataset.
    :param list columns: List of column names which should be dummy coded
    :return pandas.DataFrame: Dataset needed for training models
    """
    for col in columns:
        col = pd.get_dummies(data[col], prefix=str(col))
...
def standardize_columns(data, columns):
    ...
    for col in columns:
        col = data[col].values.reshape(1,-1)
